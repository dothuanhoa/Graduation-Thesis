...
function StudentSidebar({
  ...
  const sidebarMainNav = studentMainNav.filter(
    (item) =>
      item.path !== "/student/profile" &&
      (canScanAttendance || item.path !== "/checker/scan"),
  );
  ...
  return (
    ...
    >
      <div
        className={`mb-6 flex items-center gap-3 ${isCollapsed ? "justify-center px-0" : "px-2"}`}
      >
      ...
      {!isCollapsed && (
        <div className="mb-6 rounded-lg border border-outline-variant bg-surface-container-low p-4">
          ...
          <p className="mt-1 text-xs font-semibold text-primary">
            {studentCode || "Tài khoản sinh viên"}
          </p>
        </div>
      )}

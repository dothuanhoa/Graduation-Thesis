...
const resolveRegistrationButtonText = (
...
function StudentActivityDetailPage() {
  ...
  const registerActivity = async () => {
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Không đăng ký được hoạt động.",
      );
    ...
    return (
      <div className="space-y-gutter">
        <PageHeader
          ...
          subtitle="Hoạt động này không còn tồn tại hoặc chưa được mở cho tài khoản của bạn."
        />
        {message && (
        ...
        <BackButton to="/student/activities">
  ...
  const canRegister =
    isLimitedActivity &&
    Boolean(activity.registrationOpen) &&
  ...
  return (
    <div className="space-y-gutter">
      ...
      <PageHeader
        ...
        subtitle="Xem thời gian, địa điểm, nội dung và đăng ký hoạt động trực tiếp trên hệ thống."
      />
        ...
        {isLimitedActivity && (
          <div className="mt-6 rounded-lg border border-outline-variant bg-surface-container-low p-4">
            <p className="font-semibold text-on-surface">
              Đăng ký minh bạch trên hệ thống
            </p>
            <p className="mt-2 text-sm text-on-surface-variant">
              Danh sách đăng ký được ghi nhận tự động theo tài khoản sinh viên
              đang đăng nhập. Phòng CTSV không thêm hoặc gỡ sinh viên khỏi danh
              sách đăng ký.
            </p>
            <button

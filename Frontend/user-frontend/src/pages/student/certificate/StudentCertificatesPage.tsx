import { FilePlus2, FileText, Trash2, RefreshCw } from "lucide-react";
...
function StudentCertificatesPage() {
  ...
  const cancelRequest = async (id: string) => {
    ...
    try {
      ...
      setRequests(current => current.map(req => req.id === id ? { ...req, status: "CANCELLED" } : req));
    } catch (err) {
            ...
            <Card key={certificate.id}>
              <div className="grid gap-5 md:grid-cols-[auto_1fr_auto] md:items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-on-surface-variant space-y-1">
                    ...
                    {certificate.adminNote && (
                      <span className="block mt-1 italic text-error">Ghi chú: {certificate.adminNote}</span>
                    )}
                  </p>
                </div>

...
function StudentDetailPage() {
  ...
  const handleFaceImageUpload = async (file: File | null) => {
    ...
    setMessage("");
    try {
      const validationError = await validateFaceImageFile(file);
      if (validationError) {
            ...
            <Card>
              ...
              {profile?.faceImageUrl ? (
                <div className="mt-4 overflow-hidden rounded-xl border border-outline-variant bg-surface-container-low">
                  <img
                    alt={`Ảnh khuôn mặt mẫu của ${profile.studentId}`}
                    className="aspect-square w-full object-cover"
                    src={`${profile.faceImageUrl}?v=${faceImageVersion}`}
                  />
                </div>
              ) : (
                <div className="mt-4 flex aspect-square items-center justify-center rounded-xl border border-dashed border-outline-variant bg-surface-container-low text-on-surface-variant">

...
const runningStatuses: StudentImportJobStatus["status"][] = [
  "QUEUED",
  "PROCESSING",
];
...
function StudentImportPage() {
  ...
  const statusText = uploading
    ? "Đang tải file lên và đọc dữ liệu Excel..."
    : job?.message || "Chưa bắt đầu nhập dữ liệu.";
  ...
  const summaryItems = useMemo(
    () => [
      ...
      {
        label: "Tài khoản",
        value: job?.authTotal ? `${job.authProcessed}/${job.authTotal}` : "0/0",
      },
    ...
    const pollJob = async () => {
      try {
        ...
        if (latestJob.status === "COMPLETED") {
          setMessage(
            latestJob.message || "Nhập danh sách sinh viên thành công.",
          );
        }
        if (latestJob.status === "FAILED") {
          setMessage(
            latestJob.error ||
              latestJob.message ||
              "Nhập dữ liệu chưa hoàn tất. Vui lòng thử lại.",
          );
        }
      } catch (err) {
        if (!disposed) {
          setMessage(
            err instanceof Error
              ? err.message
              : "Không cập nhật được tiến độ nhập dữ liệu.",
          );
        }
      }
    };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        sendMail
          ? "Đã bắt đầu nhập dữ liệu và gửi email tài khoản. Bạn có thể theo dõi tiến độ bên trên."
          : "Đã bắt đầu nhập dữ liệu không gửi email tài khoản. Bạn có thể theo dõi tiến độ bên trên.",
      );
    } catch (err) {
      setMessage(
        getYupMessage(
          err,
          err instanceof Error ? err.message : "Nhập dữ liệu thất bại.",
        ),
      );
    } finally {
      setUploading(false);
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Không tải được file mẫu Excel.",
      );
    } finally {
    ...
    <>
      ...
      <Card>
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-outline-variant bg-surface-container-lowest px-4 py-3">
            <div>
              <p className="font-bold text-on-surface">
                File mẫu nhập sinh viên
              </p>
              <p className="mt-1 text-sm text-on-surface-variant">
                Tải file mẫu có sẵn cột email, ví dụ dữ liệu và từ điển
                khoa/nhóm.
              </p>
            </div>
            <button
          ...
          <label
            className={`flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-outline-variant bg-surface-container-lowest px-6 py-10 text-center transition ${
              uploading || isRunning
                ...
                : "cursor-pointer hover:border-primary hover:bg-primary-fixed"
            }`}
          >
            <FileSpreadsheet className="h-12 w-12 text-primary" />
            <span className="mt-4 text-lg font-bold text-on-surface">
              {file ? file.name : "Chọn file Excel"}
            </span>
            <span className="mt-2 text-sm text-on-surface-variant">
              Hỗ trợ .xlsx hoặc .xls, đọc cột MSSV, họ tên, email, lớp, khoa,
              niên khóa và Nhóm (1=Đầu khóa, 2=Giữa khóa, 3=Cuối khóa).
            </span>
            <input
              accept=".xlsx,.xls"
              className="sr-only"
          ...
          <label className="flex items-start gap-3 rounded-lg border border-outline-variant bg-surface-container-lowest px-4 py-3">
            ...
            <span>
              <span className="block font-bold text-on-surface">
                Gửi email tài khoản cho sinh viên sau khi nhập dữ liệu
              </span>
              <span className="mt-1 block text-sm leading-6 text-on-surface-variant">
                Mặc định đang tắt để kiểm thử nhập dữ liệu an toàn, tránh gửi mail hàng
                loạt. Nếu tắt, tài khoản vẫn được tạo/cập nhật với mật khẩu tạm
                ngẫu nhiên nhưng sinh viên sẽ không nhận email thông tin đăng
                nhập.
              </span>
            </span>
          </label>
          ...
          {message && (
            <div className="rounded-lg bg-surface-container-low px-4 py-3 font-semibold text-primary">
              {message}
            </div>
          )}

      ...
      <Card>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          ...
          <Card className="bg-surface-container-low">
            ...
            <p className="mt-2 text-sm text-on-surface-variant">
              Tạo profile và account đăng nhập.
            </p>
          </Card>
        </div>
      </Card>
    ...
    <Card>
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <FormField
          as="select"
          label="Trạng thái"
          options={["ACTIVE", "DRAFT", "PUBLISHED", "PENDING"]}
        />
        <FormField
          label="Mã tham chiếu"
          placeholder="Ví dụ: CNTT, K64-CNPM, GXN-24001"
        />
        ...
        <FormField
          ...
          placeholder="Nhập mô tả nghiệp vụ, ghi chú xử lý hoặc nội dung HTML..."
        />
      </div>
          ...
          {[
            ...
            <div key={item} className="rounded-lg bg-surface-container-low p-4">
              ...
              <p className="mt-2 font-semibold text-on-surface">
                {item === "Trạng thái" ? "Đang xử lý" : "Dữ liệu mẫu"}
              </p>
            </div>
          ))}
        </div>
        <p className="mt-6 text-on-surface-variant">
          Màn hình này dùng component chi tiết chung để hiển thị hồ sơ sinh
          viên, đơn xác nhận, hoạt động đăng ký hoặc hồ sơ khen thưởng/kỷ luật.
        </p>
      </Card>
      <Card>
            ...
            <div key={step} className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-on-surface-variant">
                  Cập nhật bởi {meta.module} lúc 09:{index}0
                </p>
              </div>
            </div>
          ))}
    ...
    <>
      ...
      <Card className="min-h-[380px]">
        <div className="flex items-center justify-between">
          ...
          <button
            className="inline-flex items-center gap-2 rounded-lg border border-outline-variant px-4 py-2 font-semibold text-primary"
            type="button"
          >
  ...
  return (
    <Card>
      <div className="grid gap-5 md:grid-cols-2">
        <FormField
          label="Tên hệ thống"
          defaultValue="Nền tảng Quản lý Công tác Sinh viên"
        />
        <FormField label="Email hỗ trợ" defaultValue="ctsv@stu.edu.vn" />
        <FormField
          as="select"
          label="Chế độ thông báo"
          options={["Email và web", "Chỉ web", "Tắt tạm thời"]}
        />
        <FormField
          as="select"
          label="Mức bảo mật"
          options={["Gateway JWT + Redis blacklist", "Gateway JWT", "Nội bộ"]}
        />
      </div>
      <div className="mt-6 flex items-center gap-3 rounded-lg bg-primary-fixed p-4 text-primary">
        <ShieldCheck className="h-5 w-5" />
        <span className="font-semibold">
          Các cài đặt này đang ở dạng cấu hình giao diện và sẽ được hoàn thiện
          theo từng nghiệp vụ.
        </span>
      </div>
    </Card>
      ...
      <Card className="bg-surface-container-low">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-on-surface-variant">
              Component màn hình đang tái sử dụng layout, bảng, form, filter,
              badge và card chung.
            </p>
          </div>
        </div>

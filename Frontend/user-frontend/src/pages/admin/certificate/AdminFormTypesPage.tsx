...
const columns: Column<FormTypeRow>[] = [
  ...
  { header: "Mô tả", key: "description" },
  {
    ...
    render: (row) => (
      <StatusBadge
        status={row.isActive === "Hoạt động" ? "ACTIVE" : "INACTIVE"}
      />
    ),
  },
...
function AdminFormTypesPage() {
  ...
  const [formData, setFormData] = useState<FormTypePayload>({
    name: "",
    formCode: "",
    description: "",
    isActive: true,
    ...
    try {
      ...
      setMessage(
        err instanceof Error
          ? err.message
          : "Không tải được danh sách loại biểu mẫu.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        err instanceof Error ? err.message : "Không lưu được loại biểu mẫu.",
      );
    } finally {
      setSaving(false);
    }
  ...
  const removeFormType = async (row: FormTypeRow) => {
    if (
      !window.confirm(
        `Xóa biểu mẫu "${row.name}"? Hành động này không thể hoàn tác.`,
      )
    )
      return;
    ...
    try {
      ...
      setFormTypes((current) =>
        current.filter((item) => String(item.id) !== row.id),
      );
      ...
      setMessage(
        err instanceof Error
          ? err.message
          : "Không xóa được loại biểu mẫu. Có thể biểu mẫu này đang được sử dụng.",
      );
    }
  };
      ...
      {(isCreating || editing) && (
        <Card>
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-primary">
                {isCreating ? "Thêm biểu mẫu mới" : "Chỉnh sửa biểu mẫu"}
              </p>
              <h2 className="text-xl font-bold text-on-surface">
                {isCreating ? "Biểu mẫu mới" : editing?.name}
              </h2>
            </div>
            <button
          ...
          <form className="grid gap-5 md:grid-cols-2" onSubmit={handleSave}>
            ...
            <FormField
              as={"select"}
              options={formCodeType}
              defaultValue={""}
              label="Mã biểu mẫu (tùy chọn)"
              onChange={(event) => updateField("formCode", event.target.value)}
          ...
          actions={(row) => (
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                onClick={() => void removeFormType(row)}
                type="button"
              >

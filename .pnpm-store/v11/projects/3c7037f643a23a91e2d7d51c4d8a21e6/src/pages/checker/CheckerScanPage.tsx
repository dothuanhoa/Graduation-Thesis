...
function CheckerScanPage() {
  ...
  const [result, setResult] = useState<FaceCheckinBatchResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const timerId = window.setTimeout(() => {
      ...
      nextUrl = URL.createObjectURL(faceFile);
  ...
  const submitFaceCheckin = useCallback(
    async (nextFaceFile: File) => {
      ...
      const validationError = await validateFaceImageFile(nextFaceFile);
      if (validationError) {
        ...
        <section className="flex min-h-[640px] min-w-0 flex-col overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-raised">
          ...
          <div className="border-t border-white/10 bg-white/[0.03] p-5">
            ...
            <input
              ...
              className="block min-w-0 max-w-full cursor-pointer rounded-xl border border-white/20 bg-slate-950/50 p-2 text-sm text-white file:mr-3 file:rounded-lg file:border-0 file:bg-primary file:px-4 file:py-2.5 file:font-bold file:text-on-primary disabled:cursor-not-allowed disabled:opacity-60"
              disabled={!hasCheckerAccess || checking}
              id="face-test-upload"
            ...
            <button
              ...
              onClick={() => {
                if (faceFile) void submitFaceCheckin(faceFile);
              }}
              type="button"
            >

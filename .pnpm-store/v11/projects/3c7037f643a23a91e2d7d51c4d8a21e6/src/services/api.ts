...
export type FaceImageImportItemResult = {
  fileName: string;
  studentId?: string;
  success: boolean;
  faceImageUrl?: string;
...
export type FaceImageBulkImportResponse = {
  total: number;
  succeeded: number;
  failed: number;
  items: FaceImageImportItemResult[];
};

export type FaceImageImportJobStatus = {
  jobId: string;
...
export type FaceCheckinBatchResponse = {
...
async function uploadFormDataWithProgress<T>(
  ...
  const send = (token: string) =>
    new Promise<{ status: number; data: unknown }>((resolve, reject) => {
      ...
      request.onerror = () => reject(new ApiError(0, toUserFacingMessage(errorMessage)));
      request.onload = () => {
        let data: unknown = request.responseText;
        try {
          data = request.responseText ? JSON.parse(request.responseText) : null;
        } catch {
          // Keep a plain-text response so the shared error formatter can display it.
        }
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : toUserFacingMessage(errorMessage);
    emitToast({ variant: "error", message });
    throw error;
  }

  if (response.status < 200 || response.status >= 300) {
    const message = toUserFacingMessage(withErrorContext(errorMessage, extractErrorMessage(response.data)));
    if (response.status !== 401) emitToast({ variant: "error", message });
    throw new ApiError(response.status, message, response.data);
  }

  onProgress(100);
  emitToast({ variant: "success", message: toSuccessMessage(successMessage) });
...
export const userApi = {
  ...
  importFaceImageFolder(files: File[]) {
    const formData = new FormData();
  ...
  startFaceImageImportJob(files: File[], onUploadProgress: (percent: number) => void = () => undefined) {
    const formData = new FormData();
    files.forEach((file) => formData.append("files", file, file.name));
    return uploadFormDataWithProgress<FaceImageImportJobStatus>(
  ...
  getFaceImageImportJob(jobId: string) {
    return apiRequest<FaceImageImportJobStatus>(`/api/users/face-images/import/jobs/${encodeURIComponent(jobId)}`, {
      suppressToast: true,
    });
  },
